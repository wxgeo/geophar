...
from wxgeometrie.geolib import (
    Point, Vecteur, Droite, Intersection, Cercle, Segment, Disque,
...
class GeolibTest(wx_unittest.TestCase):
    ...
    def test_eqn_formatee(self):
        c = Cercle(('1/2', '2/3'), 'sqrt(2)')
        self.assertEqual(
            c.equation_formatee, 'x\xb2 + y\xb2 - x - 4/3 y - 47/36 = 0')
        d = Droite(('0','0'),('1','1'))
        self.assertEqual(d.equation_formatee, '-x + y = 0')
        e=Droite(('1', '0'), ('2/3','1'))
        self.assertEqual(e.equation_formatee, '-x - 1/3 y + 1 = 0')
    ...
    def test_transformations(self):
        ...
        self.assertEqual(v1.coordonnees, tuple(h.rapport*array(v.coordonnees)))
        ...
        # le dernier appel à assert_eq échoue, avec le message :
        # 'c4.centre.coordonnees' equals to '(0.8, 0.2)', not '(4/5, 1/5)'.
        #assert_eq('c4.centre.coordonnees', '(4/5, 1/5)', l())
        # on le remplace momentanément ... par :
        assert_eq('c4.centre.coordonnees', '(0.8, 0.2)', l())


    def test_aire_diametre_perimetre(self):
